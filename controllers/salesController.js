const db = require('../config/db.manager.js');
...
exports.createNewSales = function( req, res){ // must select the body to be
raw=>JSON in Postman
...
exports.patchSalesById = function( req, res){ // must select the body to be
raw=>JSON in Postman
    const {body} = req;// req.body (attribute)
...
exports.deleteSalesByID = function( req, res){ // must select the body to be
raw=>JSON in Postman
    const {body} = req;// req.body (attribute)
    const {id} = req.params;// get id
    res.status(200).json({
